...
// encodeMsgpack serializes v to MessagePack, compresses with zstd, and
// writes the compressed bytes to w. Same encoder options as
// pkg/plugin/msgpack.go's encodeMsgpack (SetCustomStructTag("json"), zstd
// wrap) so the two codecs produce byte-identical output for the same value —
// see msgpack_test.go's golden fixture and its mirror in
