...
// Package testutil provides actor testing utilities for conformance tests.
package testutil
...
// TestCall is a generic message type for synchronous calls
...
// TestMessage is a generic message type for async messages
...
// StartTestHelperActor spawns the test helper actor on the given node.
...
// Call sends a request to a target process with the default timeout (5 seconds).
...
// Send sends an async message to a target process.
...
// ExpectMessageWithPredicate waits for a message of type T that satisfies the predicate.
