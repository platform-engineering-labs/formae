...
func newAdmissionCommandLifecycleFixture(t dstest.AdmissionLifecycleTestingT) dstest.AdmissionCommandLifecycleFixture {
	dir, err := os.MkdirTemp("", "admission-lifecycle-")
	require.NoError(t, err)
	cfg := &pkgmodel.DatastoreConfig{DatastoreType: pkgmodel.SqliteDatastore, Sqlite: pkgmodel.SqliteConfig{FilePath: filepath.Join(dir, "lifecycle.db")}}
