...
func (d *DatastoreMSSQL) Stats() (*stats.Stats, error) {
	...
	if err := d.conn.QueryRowContext(d.ctx, clientsQuery).Scan(&res.Clients); err != nil {
		return nil, err
	}

	res.Commands = make(map[string]int)
	if err := d.scanCountMap(
	...
	if err := d.conn.QueryRowContext(d.ctx, stacksQuery, string(types.OperationDelete)).Scan(&res.Stacks); err != nil {
		return nil, err
	}

	res.ManagedResources = make(map[string]int)
	if err := d.scanCountMap(fmt.Sprintf(`
...
func (d *DatastoreMSSQL) scanCountMap(query string, dst map[string]int, args ...any) error {
...
func (d *DatastoreMSSQL) GetKSUIDByTriplet(stack, label, resourceType string) (string, error) {
	...
	var ksuid string
	err := d.conn.QueryRowContext(d.ctx, query, stack, label, resourceType, string(types.OperationDelete)).Scan(&ksuid)
...
func (d *DatastoreMSSQL) UpdateResourceUpdateState(commandID string, ksuid string, operation types.OperationType, state resource_update.ResourceUpdateState, modifiedTs time.Time) error {
	...
	result, err := d.conn.ExecContext(d.ctx, query, string(state), modifiedTs.UTC(), commandID, ksuid, string(operation))
	if err != nil {
...
func (d *DatastoreMSSQL) UpdateResourceUpdateProgress(commandID string, ksuid string, operation types.OperationType, state resource_update.ResourceUpdateState, modifiedTs time.Time, progress plugin.TrackedProgress) error {
	...
	if err := d.conn.QueryRowContext(d.ctx, selectQuery, commandID, ksuid, string(operation)).Scan(&existingProgressJSON); err != nil {
	...
	result, err := d.conn.ExecContext(d.ctx, updateQuery,
...
func (d *DatastoreMSSQL) BatchUpdateResourceUpdateState(commandID string, refs []datastore.ResourceUpdateRef, state resource_update.ResourceUpdateState, modifiedTs time.Time) error {
	...
	for _, ref := range refs {
		if _, err = tx.ExecContext(d.ctx, query, string(state), modifiedTs.UTC(), commandID, ref.KSUID, string(ref.Operation)); err != nil {
