	...
	for _, policyJSON := range policies {
		...
		case "auto-reconcile":
			...
			if lastReconStr != "" && label != "" {
				part = fmt.Sprintf("AutoReconcile: %s, last %s (%s)", formatTTLDuration(duration), lastReconStr, label)
			} else if lastReconStr != "" {
				part = fmt.Sprintf("AutoReconcile: %s, last %s", formatTTLDuration(duration), lastReconStr)
			} else if label != "" {
				part = fmt.Sprintf("AutoReconcile: %s (%s)", formatTTLDuration(duration), label)
			} else {
				part = fmt.Sprintf("AutoReconcile: %s", formatTTLDuration(duration))
			}
...
func formatPolicyConfig(policyType string, configJSON json.RawMessage) string {
	...
	case "auto-reconcile":
		...
		if intervalSeconds, ok := config["IntervalSeconds"].(float64); ok {
			duration := time.Duration(int64(intervalSeconds)) * time.Second
			return fmt.Sprintf("AutoReconcile: %s", formatTTLDuration(duration))
		}
		return string(configJSON)
	default:
		return string(configJSON)
