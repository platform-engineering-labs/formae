...
func renderCancelResult(th *theme.Theme, resp *apimodel.CancelCommandResponse, _ int) string {
	...
	// Section header at column 0 (CLI convention); content indented 2, tight.
	if resp.Forced {
		fmt.Fprintf(&b, "\n%s\n", components.SectionHeader(th, "Commands have been force-canceled"))
	} else {
		fmt.Fprintf(&b, "\n%s\n", components.SectionHeader(th, "Commands are being canceled"))
	}

	// Command IDs (indent 2).
