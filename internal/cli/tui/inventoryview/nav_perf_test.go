...
// (the "⚠ unmanaged" stack cell) rendered from the current row set.
func TestNav_MovesCursorAndKeepsStyling(t *testing.T) {
	...
	// Navigation preserves the source rows and rendered cell styling.
	assert.Contains(t, mm.View(), "⚠ unmanaged")
	assert.Equal(t, before.tabs[before.active].allRows, after.tabs[after.active].allRows)
...
// A resize must flush travel and preserve the visible selection before the
// pending timer fires; Enter must then open that exact row.
func TestWheel_ResizeThenEnterUsesScrolledSelection(t *testing.T) {
	for _, size := range []tea.WindowSizeMsg{{Width: 120, Height: 24}, {Width: 90, Height: 30}} {
		mm := navModel(t, 200)
		_ = mm.View()
		var tick tea.Cmd
		for i := range 60 {
		...
		mm, _ = mm.Update(size)
		m := mm.(Model)
		require.Equal(t, 60, m.tabs[m.active].table.Cursor())
		require.Zero(t, m.wheelPending)
		require.Contains(t, mm.View(), m.tabs[m.active].table.SelectedRow()[0])
		visible, _ := m.tabs[m.active].visible(m.opts.MaxRows)
		expected := visible[60].title
		mm, _ = mm.Update(tea.KeyMsg{Type: tea.KeyEnter})
		require.True(t, mm.(Model).detailOpen)
		require.Equal(t, expected, mm.(Model).detailTitle)
		mm, _ = mm.Update(tick())
		require.Equal(t, expected, mm.(Model).detailTitle)
		require.Zero(t, mm.(Model).wheelPending)
	}
}

func TestWheel_TabSwitchDoesNotTransferPendingTravel(t *testing.T) {
	mm := navModel(t, 100)
	original := mm.(Model).active
	for range 40 {
		mm, _ = mm.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonWheelDown})
	}
	mm, _ = mm.Update(tea.KeyMsg{Type: tea.KeyTab})
	m := mm.(Model)
	require.NotEqual(t, original, m.active)
	require.Equal(t, 40, m.tabs[original].table.Cursor())
	require.Zero(t, m.wheelPending)
	before := m.tabs[m.active].table.Cursor()
	mm, _ = mm.Update(wheelSettleMsg{})
	require.Equal(t, before, mm.(Model).tabs[m.active].table.Cursor())
}

// A large upward burst can cross from a scrolled row window into the first
// screen. The selected row must remain visible across that boundary.
func TestWheel_LargeUpwardBurstKeepsSelectionVisible(t *testing.T) {
	for _, destination := range []int{0, 1, 5, 15, 30, 40} {
		mm := navModel(t, 200)
		for range 150 {
			mm, _ = mm.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonWheelDown})
		}
		mm, _ = mm.Update(wheelSettleMsg{})
		_ = mm.View()
		for range 150 - destination {
			mm, _ = mm.Update(tea.MouseMsg{Action: tea.MouseActionPress, Button: tea.MouseButtonWheelUp})
		}
		mm, _ = mm.Update(wheelSettleMsg{})
		m := mm.(Model)
		require.Equal(t, destination, m.tabs[m.active].table.Cursor())
		require.Contains(t, mm.View(), m.tabs[m.active].table.SelectedRow()[0], "destination %d", destination)
	}
}
