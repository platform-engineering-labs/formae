...
// Client reads and writes the hosted control plane on the caller's behalf.
...
// TransientError reports a definitely-temporary inability to get an
...
func (c *httpCloudClient) ListInstallations(ctx context.Context, bearer string) (Snapshot, error) {
	...
	if err != nil {
		...
		if isTimeout(err) {
			return Snapshot{}, &TransientError{Cause: err}
		}
	...
	if err != nil {
		return Snapshot{}, &TransientError{Cause: fmt.Errorf("read the installations response: %w", err)}
	}
...
func parseSnapshot(data []byte, header http.Header) (Snapshot, error) {
	envelope, repeated, err := decodeEnvelope(data, "installations response")
	if err != nil {
		return Snapshot{}, err
	}
...
// reports every member name that appeared more than once. response names the
// body in every diagnostic ("installations response", "cloud connections
// response", ...) so an error is attributed to the call that produced it.
//
...
func decodeEnvelope(data []byte, response string) (map[string]json.RawMessage, []string, error) {
	dec := json.NewDecoder(bytes.NewReader(data))
	opening, err := dec.Token()
	if err != nil {
		return nil, nil, fmt.Errorf("the %s is not valid JSON: %w", response, err)
	}
	if opening != json.Delim('{') {
		return nil, nil, fmt.Errorf("the %s is not a JSON object", response)
	}
	...
	if _, err := dec.Token(); !errors.Is(err, io.EOF) {
		return nil, nil, fmt.Errorf("the %s carries more than one JSON value", response)
	}
