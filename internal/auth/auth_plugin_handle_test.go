...
func TestAuthPluginHandle_ClientNilInitially(t *testing.T) {
	h := NewAuthPluginHandle("basic", "/path", nil)
	if h.client.Load() != nil {
		t.Fatal("expected nil client initially")
	}
}

func TestAuthPluginHandle_ValidateWithClient(t *testing.T) {
...
func TestAuthPluginHandle_ValidateWithoutClient(t *testing.T) {
	h := NewAuthPluginHandle("basic", "/path", nil)
...
func TestAuthPluginHandle_SwapClient(t *testing.T) {
	...
	// First client: valid=true
	h.SwapClient(client1)
	...
	// Swap to second client: valid=false
	h.SwapClient(client2)
...
func TestAuthPluginHandle_Close(t *testing.T) {
	...
	if h.client.Load() != nil {
		t.Fatal("expected nil client after Close")
	}
...
func TestAuthPluginHandle_RestartSwapsClient(t *testing.T) {
	// Simulate restart: old client becomes unusable, new client works after swap
	mock1 := &mockAuthPlugin{validResponse: true}
	mock2 := &mockAuthPlugin{validResponse: true}

	// Create first client manually so we can close it
	clientConn1, serverConn1 := net.Pipe()
	srv1 := rpc.NewServer()
	srv1.RegisterName("AuthPlugin", mock1)
	...
	// Close first client's connection (simulating process crash)
	clientConn1.Close()
	...
	// Swap to new client (simulating supervisor restart)
	client2 := pipeRPC(t, mock2)
	h.SwapClient(client2)
	...
	resp, err = h.Validate(&pkgauth.ValidateRequest{})
	if err != nil {
		t.Fatalf("Validate after swap failed: %v", err)
	}
	if !resp.Valid {
		t.Fatal("expected valid=true after swap")
	}
}
