...
func NewDatastoreSQLite(ctx context.Context, cfg *pkgmodel.DatastoreConfig, agentID string) (Datastore, error) {
	...
	_, err = d.conn.Exec(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (command_id TEXT NOT NULL, timestamp TEXT NOT NULL, command TEXT NOT NULL, state TEXT NOT NULL, agent_version TEXT, client_id TEXT, agent_id TEXT, data TEXT, PRIMARY KEY (command_id))", CommandsTable))
	if err != nil {
	...
	_, err = d.conn.Exec(`CREATE TABLE IF NOT EXISTS resources (
		uri TEXT NOT NULL,
	...
	// Composite index for batch triplet lookups
	...
	_, err = d.conn.Exec(`CREATE TABLE IF NOT EXISTS targets (
		label TEXT NOT NULL,
...
func (d DatastoreSQLite) createIndex(tableName, columnName string) error {
	indexName := fmt.Sprintf("idx_%s", columnName)
	_, err := d.conn.Exec(fmt.Sprintf("CREATE INDEX IF NOT EXISTS %s ON %s (%s)", indexName, tableName, columnName))
	if err != nil {
...
func (d DatastoreSQLite) createCompositeIndex(tableName string, columns []string) error {
	indexName := fmt.Sprintf("idx_%s_%s", tableName, strings.Join(columns, "_"))
	columnList := strings.Join(columns, ", ")
	_, err := d.conn.Exec(fmt.Sprintf("CREATE INDEX IF NOT EXISTS %s ON %s (%s)", indexName, tableName, columnList))
	if err != nil {
...
func (d DatastoreSQLite) storeResource(resource *pkgmodel.Resource, data []byte, commandID string, operation string) (string, error) {
	...
	if managed == 0 && operation != string(resource_update.OperationDelete) {
		query = `DELETE FROM resources WHERE ksuid = ?`
		_, err = d.conn.Exec(query, ksuid)
		if err != nil {
