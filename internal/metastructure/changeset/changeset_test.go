...
func TestChangeset_CrashRecovery_SuccessParentBlocksChildren(t *testing.T) {
	...
	cs, err := NewChangesetFromResourceUpdates(allUpdates, "test-crash-bug", pkgmodel.CommandApply)
	require.NoError(t, err)
...
func TestChangeset_CrashRecovery_FilteredPendingUpdatesOnly(t *testing.T) {
	...
	cs, err := NewChangesetFromResourceUpdates(pendingOnly, "test-crash-fix", pkgmodel.CommandApply)
	require.NoError(t, err)
	...
	assert.Equal(t, "test-subnet", updates[0].DesiredState.Label)
}
