...
func TestResourceUpdater_NotFoundDuringSync_DeleteOperation(t *testing.T) {
	testutil.RunTestFromProjectRoot(t, func(t *testing.T) {
		readCalled := false
		deleteCalled := false
		...
		overrides := &plugin.ResourcePluginOverrides{
			...
			Delete: func(request *resource.DeleteRequest) (*resource.DeleteResult, error) {
				deleteCalled = true
				t.Fatal("Delete should not be called for NotFound resources")
				return nil, nil
			},
		}
		...
		_, err = m.Datastore.StoreStack(&pkgmodel.Forma{
		...
		res := &pkgmodel.Resource{
			Label:      "test-vpc",
			Type:       "FakeAWS::EC2::VPC",
			Properties: json.RawMessage(`{"VpcId":"vpc-123"}`),
			Stack:      "test-stack",
			Schema:     pkgmodel.Schema{Fields: []string{"VpcId"}},
			Target:     "test-target",
			Ksuid:      util.NewID(),
		}
		_, err = m.Datastore.StoreResource(res, "test")
		assert.NoError(t, err)

		commandID := "test-command-notfound-delete"
		...
		testutil.Call(m.Node, "FormaCommandPersister", forma_persister.StoreNewFormaCommand{
			Command: forma_command.FormaCommand{
				ID:              commandID,
				State:           forma_command.CommandStateNotStarted,
				ResourceUpdates: []resource_update.ResourceUpdate{resourceUpdate},
			},
		})
		...
		_, err = testutil.Call(m.Node, "ChangesetSupervisor", changeset.EnsureChangesetExecutor{CommandID: commandID})
		assert.NoError(t, err)

		testutil.Send(m.Node, actornames.ChangesetExecutor(commandID), changeset.Start{Changeset: cs})
		...
		assert.True(t, readCalled, "Read should have been called to check resource state")
		assert.False(t, deleteCalled, "Delete should not be called for NotFound resource")

		// Stack and resource should still exist - NotFound during sync treats resource as already deleted
		stacks, err := m.Datastore.LoadAllStacks()
		assert.NoError(t, err)
		assert.Len(t, stacks, 1, "Stack should still exist")
		assert.Empty(t, stacks[0].Resources, "Resource should be removed from stack after NotFound during sync")
	})
}

func TestResourceUpdater_NotFoundDuringSync_CreateOperation(t *testing.T) {
	testutil.RunTestFromProjectRoot(t, func(t *testing.T) {
		readCalled := false
		createCalled := false
		...
		commandID := "test-command-notfound-create"
		resourceUpdate := resource_update.ResourceUpdate{
			DesiredState: pkgmodel.Resource{
				Label:      "test-vpc",
				Type:       "FakeAWS::EC2::VPC",
		...
		assert.True(t, readCalled, "Read should be called to check current state")
		assert.True(t, createCalled, "Create should proceed after NotFound during sync")

		stacks, err := m.Datastore.LoadAllStacks()
		assert.NoError(t, err)
		assert.Len(t, stacks, 1, "Stack should exist after successful create")
		assert.Len(t, stacks[0].Resources, 1)
		assert.Equal(t, "vpc-created", stacks[0].Resources[0].NativeID)
	})
}
