...
func TestChangesetExecutor_CancelInProgress(t *testing.T) {
	testutil.RunTestFromProjectRoot(t, func(t *testing.T) {
		// Create a long-running resource operation that uses Status polling
		// Create returns InProgress, then Status indefinitely returns InProgress
		// This ensures the ResourceUpdater is truly in progress when we cancel
		createStarted := make(chan struct{})

		overrides := &plugin.ResourcePluginOverrides{
			Create: func(request *resource.CreateRequest) (*resource.CreateResult, error) {
				close(createStarted) // Signal that create has started
				// Return InProgress to trigger status polling
				return &resource.CreateResult{
					ProgressResult: &resource.ProgressResult{
						Operation:       resource.OperationCreate,
			...
			Status: func(request *resource.StatusRequest) (*resource.StatusResult, error) {
				// Indefinitely return InProgress - the operation never completes on its own
				// This ensures we're testing cancellation during an ongoing operation
		...
		select {
		case <-createStarted:
			// Good, create has started
		case <-time.After(5 * time.Second):
			t.Fatal("Create operation never started")
		}

		// Give the PluginOperator time to start polling Status
		// This ensures we're cancelling during an active operation, not just after Create returned
		time.Sleep(100 * time.Millisecond)

		// Send cancel message while the resource operation is in progress
		testutil.Send(m.Node, actornames.ChangesetExecutor(commandID), changeset.Cancel{
			CommandID: commandID,
		})

		// Wait for cancellation to complete
		testutil.ExpectMessageWithPredicate(t, messages, 10*time.Second, func(msg changeset.ChangesetCompleted) bool {
			return msg.CommandID == commandID && msg.State == changeset.ChangeSetStateCancelled
		})

		// Verify the command was cancelled in the database
		commandRes, err := testutil.Call(m.Node, "FormaCommandPersister", forma_persister.LoadFormaCommand{
			CommandID: commandID,
		})
		assert.NoError(t, err)
		...
		command, ok := commandRes.(*forma_command.FormaCommand)
		assert.True(t, ok)
		assert.Equal(t, forma_command.CommandStateCanceled, command.State)

		// The resource update should be marked as cancelled
		assert.Len(t, command.ResourceUpdates, 1)
		assert.Equal(t, resource_update.ResourceUpdateStateCanceled, command.ResourceUpdates[0].State)

		// State transition verification:
		// - ChangesetExecutor successfully transitioned to StateCanceled (proven by ChangesetCompleted message)
		// - Command marked as Canceled in database (verified above)
		// - Resource marked as Canceled in database (verified above)
		//
		// Note: The Ergo state machine library automatically logs: "StateMachine: switching to state 'canceled'"
		// Additional runtime verification (not critical for functional correctness):
		// - ResourceUpdater receives Shutdown message and terminates (happens via supervisor cleanup)
		// - PluginOperator receives cancellation signal from ResourceUpdater termination
	})
}
