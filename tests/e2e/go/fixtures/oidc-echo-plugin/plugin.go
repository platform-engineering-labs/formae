...
const (
	// audience the plugin asks its broker for. Any value works: the stub
	// broker echoes it back inside the token it mints.
...
// tokenProperties asks the broker for a token and renders the resource's
// properties around the outcome. A failure is recorded in tokenError rather
// than failed outright, so a test asserting the unpaired case reads an exact
// message instead of whatever an operator error renders to. Real plugins fail
// closed here; this one is only proving the wiring.
...
// Read reports the same document Create did, minting a fresh token: there is
// no store behind this resource, and the stub broker's token is a pure
